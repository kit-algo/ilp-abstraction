...
class GurobiInterface
    : public Interface<GRBVar, GRBLinExpr, grb_internal::ConstraintPair,
...
public:
	...
	public:
		...
		/*
		void enable_kappa_statistics() noexcept;
	...
	private:
		...
		// Arithmetic types
		template <class LowerValType>
		GRBConstr add_lower_constraint(
		    std::enable_if_t<std::is_arithmetic<LowerValType>::value> lower_bound,
		    Expression expr, std::string name);

		// Exrpessions
		inline GRBConstr add_lower_constraint(Expression lower_bound,
		                                      Expression expr, std::string name);

		// Disable
		inline GRBConstr add_lower_constraint(DummyValType lower_bound,
		                                      Expression expr, std::string name);

		// Arithmetic types
		template <class UpperValType>
		GRBConstr add_upper_constraint(
		    std::enable_if_t<std::is_arithmetic<UpperValType>::value> upper_bound,
		    Expression expr, std::string name);

		// Exrpessions
		inline GRBConstr add_upper_constraint(Expression upper_bound,
		                                      Expression expr, std::string name);

		// Disable
		inline GRBConstr add_upper_constraint(DummyValType upper_bound,
		                                      Expression expr, std::string name);

		ModelStatus status;

		// Kappa Stats
		bool collect_kappa = false;
		double root_kappa;
		double presolved_kappa;
	...
	features()
	{
		return Features::FeatureList<Features::IIS>{};
	}
...
#ifndef ILP_ABSTRACTION_COMMON_CPP
