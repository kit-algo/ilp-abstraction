...
#define ILP_ABSTRACTION_CPLEX_CPP
...
set_param_on_cplex(IloCplex & cplex, ParamType type, T val)
{
	...
	case ParamType::NODE_FILE_START:
		cplex.setParam(IloCplex::Param::MIP::Strategy::File,
		               3); // write node files on disk and compressed
		cplex.setParam(IloCplex::Param::WorkMem,
		               1024 * (double)val); // conversion to megabytes
		break;
	default:
		assert(false);
	}
...
template <>
inline void
set_param_on_cplex<const char *>(IloCplex & cplex, ParamType type,
                                 const char * val)
{
	switch (type) {
	case ParamType::NODE_FILE_DIR:
...
template <>
inline void
set_param_on_cplex<ParamMIPFocus>(IloCplex & cplex, ParamType type,
                                  ParamMIPFocus val)
{
...
CPLEXInterface::~CPLEXInterface() { this->env.end(); }
...
{
	this->cplex.setParam(IloCplex::Param::MIP::Strategy::KappaStats,
	                     1); // TODO make 1 or 2 configurable
}
...
CPLEXInterface::Model::kappa_stats()
{
	...
	double suspicious =
	    this->cplex.getQuality(IloCplex::Quality::KappaSuspicious, -1);
	double unstable =
	    this->cplex.getQuality(IloCplex::Quality::KappaUnstable, -1);
	double illposed =
	    this->cplex.getQuality(IloCplex::Quality::KappaIllposed, -1);
	double kappamax = this->cplex.getQuality(IloCplex::Quality::KappaMax, -1);
	double attention =
	    this->cplex.getQuality(IloCplex::Quality::KappaAttention, -1);

	return {stable,   suspicious, unstable,     illposed,
	        kappamax, attention,  std::nullopt, std::nullopt};
}
...
{
	this->start_values.insert(
	    std::make_pair<long, IloNum>(var.getId(), IloNum(val)));
}
...
CPLEXInterface::Model::apply_start_solution()
{
	...
	for (auto & entry : this->start_values) {
		IloNumVar var(
		    (IloNumVarI *)this->interface->env.getExtractable(entry.first));
		startVar.add(var);
...
CPLEXInterface::Model::set_objective(Expression expr, ObjectiveType type)
{
	...
	auto sense = (type == ObjectiveType::MAXIMIZE) ? IloObjective::Maximize
...
void
CPLEXInterface::Model::change_constraint_ub(Constraint & constr,
...
void
CPLEXInterface::Model::change_constraint_lb(Constraint & constr,
...
void
CPLEXInterface::Model::change_var_bounds(Variable & var,
                                         LowerValType lower_bound,
                                         UpperValType upper_bound)
{
...
void
CPLEXInterface::Model::change_objective_coefficient(Variable & var,
                                                    double coefficient)
{
                                      ...
                                      std::string name)
{
	...
	IloRange lower_constr =
	    IloRange(this->interface->env, 0, expr - lower_bound, IloInfinity);
	this->m.add(lower_constr);

	IloRange upper_constr(this->interface->env, -1 * IloInfinity,
	                      expr - upper_bound, 0);
...
CPLEXInterface::Model::CallbackAdapter::CallbackAdapter(IloEnv env_in,
                                                        Model * model_in)
